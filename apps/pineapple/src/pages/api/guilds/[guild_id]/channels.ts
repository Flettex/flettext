    ...
    try {
        ...
        if (req.method === "POST") {
            ...
            if (req.body.position && req.body.id) {
                ...
                // 0x2 Admin
                // 0x4 MANAGE_CHANNELS
                if (member && (((member?.permissions & 0x4) === 0x4) || ((member?.permissions & 0x2) === 0x2))) {
                    const channel = await prisma.channel.findUnique({
                        where: {
                            id: req.body.id
                        }
                    });
                    if (channel && channel.position && (req.body.position - channel.position > 0)) {
                        ...
                        [...updated[0].map((item) => ({...item, position: (item?.position || 1)-1})), updated[2]];
                    } else if (channel && channel.position && (req.body.position - channel.position < 0)) {
                        ...
                        res.status(205).end();
                        // send json
                        [...updated[0].map((item: Channel): Channel => ({...item, position: (item.position || 1) - 1})), updated[2]];
                    } else {
                        res.status(400).json({ message: "Invalid position or channel" })
                    }
                } else {
                    res.status(403).json({ message: "Member or channel was not found or missing permissions" })
                }
            }
        } else {
