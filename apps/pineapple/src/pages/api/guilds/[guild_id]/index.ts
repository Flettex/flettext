...
export default async function Get(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (req.method === "GET") {
            ...
            if ((req.body.name && req.body.name.length > 0) || req.body.owner_id) {
                ...
                if (member && (((member?.permissions & 0x1) === 0x1) || ((member?.permissions & 0x2) === 0x2)) || (req.body.owner_id && user.id === data?.ownerId)) {
                    ...
                    res.status(403).json({ message: "Member was not found or missing permissions" });
                }
            } else {
                res.status(400).json({ message: "Not enough body parameters or invalid" });
            }
